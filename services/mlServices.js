...
class PerformanceAnalyzer {
    ...
    preprocessData(timeTrackingData) {
        try {
            ...
            const stats = this.calculateStats(timeTrackingData);

            return timeTrackingData.map(entry => {
                if (!entry) throw new Error('Invalid entry in data');
                ...
                return {
                    features: [
                        ...
                        (overtimeHours - stats.overtimeHours.mean) / stats.overtimeHours.std,
                        isHoliday
                    ],
    ...
    calculateStats(data) {
        ...
        for (const feature in stats) {
            ...
            stats[feature].std = Math.sqrt(
                values.reduce((a, b) => a + Math.pow(b - stats[feature].mean, 2), 0) / values.length
            ) || 1;
        }
    ...
    calculatePerformanceScore(entry) {
        ...
        const onTimeScore = entry.minutes_late === 0 ? 1 : Math.max(0, 1 - entry.minutes_late / 60);
        const hoursScore = Math.min(1, this.parseHours(entry.total_hours) / 8);
        const overtimeScore = Math.min(1, this.parseHours(entry.overtime_hours) / 4);
            ...
            const history = await this.model.fit(xs, ys, {
                ...
                callbacks: {
                    onEpochEnd: (epoch, logs) => {
                        console.log(`Epoch ${epoch}: loss = ${logs.loss.toFixed(4)}, accuracy = ${logs.acc.toFixed(4)}, val_loss = ${logs.val_loss.toFixed(4)}`);
                    }
                }
            });
    ...
    async analyzeWithAI(timeTrackingData) {
        try {
                ...
                Object.entries(employeeData).map(async ([employeeId, entries]) => {
                    try {
                        ...
                        return {
                            ...
                            metrics: {
                                attendance_rate: entries.filter(e => e.minutes_late === 0).length / entries.length,
                                avg_work_hours: entries.reduce((acc, e) => acc + this.parseHours(e.total_hours), 0) / entries.length,
                                avg_overtime: entries.reduce((acc, e) => acc + this.parseHours(e.overtime_hours), 0) / entries.length,
            ...
            return {
                ...
                averageScore: validPredictions.reduce((acc, p) => acc + p.performance_score, 0) / validPredictions.length,
    ...
    calculateDepartmentStats(predictions) {
        return predictions.reduce((acc, p) => {
            ...
            if (!acc[position].topPerformer || p.performance_score > acc[position].topPerformer.performance_score) {
